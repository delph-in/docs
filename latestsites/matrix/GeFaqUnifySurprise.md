...
- (Applicable to ace only, not the LKB): If the value of `parsing-roots` in `ace/config.tdl` is a string that does not correspond to something in `roots.tdl`, ace will refuse to put any edges into the spanning cell of the chart, even if they can be built interactively. Discovered in Olomouc, during [KRG discussion](https://delph-in.github.io/docs/matrix/OlomoucKoreanGrammar)
