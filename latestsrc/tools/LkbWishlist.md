...
you. In some cases, there has been relevant discussion on DELPH-IN
Discourse (e.g., non-ASCII encodings).
...
- **Treebanking support**: [LkbFos](https://delph-in.github.io/docs/tools/LkbFos) can now output full parse
forests in \[incr tsdb()\] format. However, still no support
for connection to \[incr tsdb()\] itself or to FFTB.
- **Shared configuration files**: LkbFos now supports the
TDL-inspired DELPH-IN configuration file format (e.g. for the ERG,
via config.tdl and english.tdl).
...
rules. In LkbFos, set the parameter
\*show-incomplete-lex-rule-chains\* to NIL to stop such
lexical items being displayed.

Last update: 2026-07-25 by John Carroll [[edit](https://github.com/delph-in/docs/wiki/LkbWishlist/_edit)]{% endraw %}