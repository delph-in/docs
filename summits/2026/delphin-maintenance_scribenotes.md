...
Ecosystems:
* LKB: [incr tsdb()] (graphical component)
* ACE: ART, FTTB
* Pydelphin
...
John: Woodley is the only one for LUI \
Luis: No Metal Mac version of YZ LUI? \
John: x86 LUI executable that should work on Rosetta Macs \
...
Dan: PET is another parser from late 90s, supplanted by ACE, which could also do generation and has more debugging tools. PET has been on the sideline since the 2000s. Probably no reason to keep PET running. 2025 ERG still has PET subdirectory, but docstrings cause PET compiler to break if not stripped out \
...
Emily/Dan: PET is written in C++, ACE in C \
...
John: Was too fragile so did not release it, SDL 2 is on the horizon, still possible in the future \
...
Dan: Maybe take display/graphic aspects off of John’s plate and hand over to Luis \
...
John: Full forest is one of the profiles that can be generated by [incr tsdb()] \
John: Full forest is the edges, contains start index, end index, packing information, lexical type \
[INCOMPLETE] Emily: alternative is syntactic determinants over … \
[INCOMPLETE] John: can get richer version using the MRSs

### Integration of preprocessing steps
Luis: Wants POS tagger functionality to be grammar agnostic, only works for ERG in ACE \
...
John: External preprocessor makes more sense, should not have a POS tagger in ACE \
...
### DELPH-IN docs
John: Moving back to LKB, about wiki pages, there are pages about the state of LKB/[incr tsdb()] 10 years ago and how to use it that are no longer relevant, want to delete them and repurpose the relevant ones to point to LKB-FOS \
